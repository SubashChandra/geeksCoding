...
void postorder(Bstnodeptr root)
{
	...
	while(1)
	{
		...
		{
			if(s1.size()==0) //if empty stack 
			{
				return;
			}

			else if(s1.top()->right==NULL) //if leaf
			{
				root=s1.top();
				s1.pop();
				...
				printf("%d ",root->data);
				while(s1.size()!=0 && s1.top()->right==root) //if root is the right child of node on top of stack
				{
					root=s1.top();
			...
			if(s1.size()!=0) //now go process right child
				root=s1.top()->right;
			else
				return;
		}
...
void postorder1(Bstnodeptr root)
{
	if(root==NULL)
		return;
	stack<Bstnodeptr>s1;
	...
	while(s1.size()!=0)
	{
		...
		s2.push(root); //psuh onto second stack
	...
	//now pop nd print all elemenst of stack2
	while(s2.size()!=0)
	{
