//print the deepest left leaf node is a bst
//O(n) time
//O(logn) time
...
void leafUtil(Bstnodeptr root, int level,int *maxLevel, int *deepLeaf, int flag)
{
	if(root) 
	{
		if(root->left==NULL && root->right==NULL && flag && level>*maxLevel)
		{
			*maxLevel=level;
			*deepLeaf=root->data;
		}

		leafUtil(root->left, level+1, maxLevel, deepLeaf, 1); //flag set 1 for left child only
		leafUtil(root->right, level+1, maxLevel, deepLeaf, 0);
	}
}



int deepestLeftLeaf(Bstnodeptr root)
{
	if(root==NULL)
		return -1;
	...
	leafUtil(root,level,&maxLevel,&deepLeaf,1);
	return deepLeaf;
}
