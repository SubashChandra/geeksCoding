//given a no find its ceil in Bst
//O(log n) time
//O(log n) space
...
int getFloor(Bstnodeptr root, int data)
{
	if(root==NULL)
		return 10000;

	if(root->data==data) //floor foudn
		return data;

	else if(data<root->data) //go to left
		return getFloor(root->left, data);

	int floor=getFloor(root->right,data); //go floor, ceck if found
	
	return floor<=data?floor:root->data; //if found return it, else return root->data
}
