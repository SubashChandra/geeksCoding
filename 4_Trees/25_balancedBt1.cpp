//given a bt, check if it sis balanced or not
//O(n^2) time (height is calculated for every node)
...
int isBalanced(Bstnodeptr root)
{
	int lh,rh;
	if(root==NULL)
		return 1; //balanced
	...
	return abs(lh-rh)<=1 && isBalanced(root->left) && isBalanced(root->right); //go recursive
}
