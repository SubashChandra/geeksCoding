...
int getCeil(Bstnodeptr root, int data)
{
	if(root==NULL)
		return -1;

	if(root->data==data) //ceil foudn
		return data;

	else if(data>root->data) //go to right
		return getCeil(root->right, data);

	int ceil=getCeil(root->left,data); //go left, ceck if found
	
	return ceil>=data?ceil:root->data; //if found return it, else return root->data
}
