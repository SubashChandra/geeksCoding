...
Bstnodeptr utilFunc(Bstnodeptr root)
{
	...
	if(root->left!=NULL)
	{
		...
		left=utilFunc(root->left);
		for( ;left->right!=NULL; left=left->right); //inorder predecessor of root ie rightmost of left child
	...
	if(root->right)
	{
		...
		right=utilFunc(root->right);
		for( ;right->left!=NULL;right=right->left); //inorder successor of root ie leftmost of right child
