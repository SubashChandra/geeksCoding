...
void printUtil(Bstnodeptr root, int level, int *maxLevel)
{
	if(root)
	{
		if(*maxLevel < level) //if first node of higher level
		{
			printf("%d ",root->data);
			*maxLevel=level;
		}

		printUtil(root->left,level+1,maxLevel);
		printUtil(root->right,level+1,maxLevel);
	}
}


void printLeftview(Bstnodeptr root)
{
	if(root==NULL)
		return;

	int maxLevel=0;
	printUtil(root,1,&maxLevel);
	printf("\n");
}
